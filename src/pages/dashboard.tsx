...
import { getVehicleUsages } from 'apiSdk/vehicle-usages';
...
import { VehicleListPage as ShowCaseList } from 'pages/vehicles';

function DashboardPage() {
  const { session } = useSession();

  const vehicleUsageStat = useSWR<PaginatedInterface<VehicleUsageInterface>>(
    () => `/vehicle-usages`,
    () => getVehicleUsages({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: vehicleUsageStat?.data?.totalCount,
      isLoading: vehicleUsageStat.isLoading,
